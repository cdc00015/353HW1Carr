...
ROUND(CAST(SUM (G.HomePoints + G.AwayPoints) AS FLOAT) / NULLIF(CAST(COUNT(G.GameID) AS INT) ,0) ,2) AS "Points Per Game",
ROUND(CAST(SUM (G.HomeYards + G.AwayYards) AS FLOAT) / NULLIF(CAST(COUNT(G.GameID) AS INT) ,0) ,2) AS "Yards Per Game"
from Team as T
