...
ROUND(CAST(SUM (G.HomePoints + G.AwayPoints) AS FLOAT) / NULLIF(CAST(COUNT(G.GameID) AS INT) ,0) ,2) AS "Points Per Game", --Used ChatGPT.  Prompt used was: "With the current database layout and data, how would I make a standard procedure to calculate the average points by a team per game?"
